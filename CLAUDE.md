...
**Figure 2: model-structure figure (SVG via `tools/structure_figure.py`).** Reference style is the Keras v3 figure the repo owner provided, and the owner wants it matched closely: dark navy panel, title inside the panel, one rounded container with a green border, gray-bordered boxes with a single short name each, red text for the component that can execute code (Keras: the Lambda layer; PyTorch: the pickle `GLOBAL + REDUCE / NEWOBJ / BUILD` box), and nothing else.
...
- Layout: labelled column = a section box with stacked single-line boxes (the "metadata / program" half, label is the bare file name such as `data.pkl`); unlabelled column = one tall box (the "weights" half). Two boxes carry short extra lines because the owner asked for them: the weights box (`Tensor Storages` / `data/0 … data/N` / `one per storage`) and the red box (`GLOBAL + REDUCE / NEWOBJ / BUILD` / `any importable callable  (blocked by weights_only)`).
- Otherwise names only. No other captions: no second line inside the other program boxes, no `container_label`, no footer row of bookkeeping records, no `caption` under the panel. The generator still supports all of these, but the owner asked for them to be removed from the PyTorch figure to match the example image; bookkeeping records belong in the README's record table instead.
...
- GitHub caches README images by URL (camo proxy), so a regenerated figure at the same path can keep showing the old version for a long time. The README therefore embeds the SVG with a cache-busting suffix, `figures/<fw>_structure.svg?v=<short blob hash>`. After regenerating a figure, update the suffix: `git hash-object <fw>/figures/<fw>_structure.svg | cut -c1-7`.
- Commands:
