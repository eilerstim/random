...
- Layout: labelled column = a section box with stacked single-line boxes (the "metadata / program" half, label is the bare file name such as `data.pkl`); unlabelled column = one tall box (the "weights" half, named like the example: `Model Weights (data/N)`).
- Names only. No captions of any kind: no second line inside a box, no `container_label`, no footer row of bookkeeping records, no `caption` under the panel. The generator still supports all of these, but the owner asked for every caption to be removed from the PyTorch figure to match the example image; bookkeeping records belong in the README's record table instead.
