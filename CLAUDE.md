...
- Layout: labelled column = a section box with stacked single-line boxes (the "metadata / program" half, label is the bare file name such as `data.pkl`); unlabelled column = one tall box (the "weights" half). The weights box is the one place a short second and third line are wanted: the entry pattern and the cardinality (`Tensor Storages` / `data/0 … data/N` / `one per storage`), which the owner asked to keep.
- Otherwise names only. No other captions: no second line inside the program boxes, no `container_label`, no footer row of bookkeeping records, no `caption` under the panel. The generator still supports all of these, but the owner asked for them to be removed from the PyTorch figure to match the example image; bookkeeping records belong in the README's record table instead.
